...
# Constants, see doc string for how they're used in VariableSizeImageTransform
IMAGE_RES = 224
...
def _stack_images(
    ...
    for imgs_sample in images:
        out_images_i = torch.zeros(
            (max_num_images, max_num_chunks, 3, image_res, image_res),
            dtype=getattr(config, "torch_dtype", None)
            or getattr(
                config.neuron_config, "torch_dtype", None
            ),  # temporary fix since vllm passes config directly during benchmarking
        )
        _num_chunks = []
...
def custom_image_preprocessing(
    config: InferenceConfig,
    batch_images: List[List[PIL_Image.Image]],
):
    """
    adapted from Meta's original Llama3.2 Multimodal PyTorch implementation
    CrossAttentionTransformer.compute_vision_tokens_masks()
    """
    max_num_images = max(len(x) for x in batch_images)
    bsz = len(batch_images)

    image_transform = partial(
        VariableSizeImageTransform(size=config.vision_config.image_size),
        max_num_chunks=config.vision_config.max_num_tiles,
    )
    images_and_aspect_ratios = [[image_transform(im) for im in row] for row in batch_images]
    ...
    stacked_images, num_chunks = _stack_images(
        transformed_images,
        max_num_chunks=config.vision_config.max_num_tiles,
        image_res=config.vision_config.image_size,
        max_num_images=max_num_images,
        config=config,
    )
    num_chunks = torch.tensor(num_chunks)
    return stacked_images, aspect_ratios, num_chunks
