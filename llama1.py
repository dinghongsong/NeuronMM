...
def nki_thin_gemm(lhsT, rhs):
    ...
    if N >=2048:
        ...
        micron_kernel(M, 128, 512, 1, 4, 0, first_n_end)
        # print("first_n_end", first_n_end)
        # print("N", N)
        if first_n_end < N:
            micron_kernel(M, 128, 128, 1, 1, first_n_end, N)
    elif N == 256:
        micron_kernel(M, 128, N, 1, 1, 0, N)
    elif N == 512:
        micron_kernel(M, 128, 512, 1, 1, 0, N)
    elif N == 1024:
        micron_kernel(M, 128, 512, 1, 2, 0, N)
    elif N == 1536:
        micron_kernel(M, 128, 512, 1, 3, 0, N)
    else:
        assert False
    return result
...
def custom_gemx_implement(input_parallel, weight):
    ...
    if bsz <= 128:
        ...
        # output_parallel = nki_matmul_fully_optimized_(input_parallel.T,weight,TILES_IN_BLOCK_M1=1,TILES_IN_BLOCK_N=n,TILES_IN_BLOCK_K=k)
...
class CustomFusedColumnParallelLinear(ColumnParallelLinear):
    ...
    def forward(self, input: torch.Tensor, rmsnorm, residual, *_: Any) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        if not NKI_ENABLED:
            ...
            bsz = input_parallel.shape[1] * input_parallel.shape[0]
            if bsz <= 128 and not CONFIG_MLP_FUSE_NONE:
