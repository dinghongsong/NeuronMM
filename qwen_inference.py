...
from models.qwen3.modeling_qwen3 import NeuronQwen3ForCausalLM
...
def svd_flash(args):
    ...
    for i in tqdm(range(len(layers))):
        ...
        if "Llama" in args.model_path or "llama" in args.model_path or "vicuna" in args.model_path:
            ...
            svd_mlp = SVD_LlamaMLP(config=model.config, compress_ratio=args.compress_ratio) # can reuse LlamaMLP module
...
if __name__ == "__main__":
    args = parse_args()
    report_wo_svd = run_inference(NeuronQwen3ForCausalLM, args, svd=False)
    svd_flash(args)
    report_svd = run_inference(NeuronQwen3ForCausalLM, args, svd=True)
